...
function build {
  ...
  *)
  ...
  pushd "$SCRIPTDIR/.."
  $VCPKG_ROOT/vcpkg install --triplet $VCPKG_TARGET --x-install-root="$VCPKG_ROOT/installed" || true
  echo "===BEGIN issue_body.md==="
  cat /opt/artifacts/vcpkg/installed/vcpkg/issue_body.md
  echo "===END issue_body.md==="
  popd
